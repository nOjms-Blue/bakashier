...
func ExportBks(name string, reader func(length uint64) ([]byte, error), writer func(data []byte) error, password string, chunkSize uint64) error {
	...
	if err := writer([]byte("BKS")); err != nil {
		return err
	}
	if err := writer(versionBytes); err != nil {
		return err
	}
	if err := writer(nameLenBytes); err != nil {
		return err
	}
	if err := writer(nameBytes); err != nil {
		return err
	}
	if err := writer(nameCRC); err != nil {
		return err
	}
	...
	for {
		...
		if err := writer(chunkLenBytes); err != nil {
			return err
		}
		if err := writer(chunk); err != nil {
			return err
		}
		if err := writer(chunkCRC); err != nil {
			return err
		}
	}
...
func ImportBks(reader func(length uint64) ([]byte, error), writer func(name string, data []byte) error, password string) error {
	...
	for {
		...
		if err := writer(name, chunk); err != nil {
			return err
		}
		wroteChunk = true
	}
	...
	if !wroteChunk {
		if err := writer(name, []byte{}); err != nil {
			return err
		}
	}
